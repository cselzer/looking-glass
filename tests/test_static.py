...
class StaticFilesTests(unittest.TestCase):
    ...
    def test_xss_hardening(self):
        ...
        self.assertIn("window.setSafeHref", gui_text)
        self.assertIn("window.setSafeHref", page)
        self.assertNotRegex(gui_text, r"(?<!\.)setSafeHref\s*\(")
        ...
        head, _, bar = page.partition('id="status-bar"')
        self.assertNotIn('id="status-ip"', head)
        self.assertIn('id="status-ip"', bar)
        self.assertIn("majfault", gui_text)
        self.assertIn("⬇️", gui_text)
